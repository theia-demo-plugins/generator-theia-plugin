...
export function initQuickPickCommands(disposables: theia.Disposable[]) {
    ...
    disposables.push(theia.commands.registerCommand({id: PLUGIN_NAME + 'quick-input', label: COMMAND_NAME_PREFIX + 'Quick Input'},()=>{
        theia.window.showInputBox({prompt:"Hello Plugin",
                                    ...
                                    validateInput: s=>{
                                        return new Promise(r=>{
                                            if(s.length <=3){
                                                r('Input should be longer than 3 chars');
                                            } else{
                                                r(undefined);
                                            }
                                        })
                                    }}).then(s =>{
            theia.window.showInformationMessage(typeof s !== 'undefined'? `Entered: ${s}`: "Input was canceled!");
        });
    }));
}
